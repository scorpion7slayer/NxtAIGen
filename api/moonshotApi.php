...
if (empty($MOONSHOT_API_KEY)) {
  http_response_code(500);
  echo json_encode(['error' => 'Clé API Moonshot non configurée']);
  exit();
}
...
if (!isset($input['model']) || empty($input['model'])) {
  ...
  $modelsData = getMoonshotModels($MOONSHOT_API_KEY);
  ...
  if (isset($modelsData['models']) && !empty($modelsData['models'])) {
    ...
    http_response_code(500);
    echo json_encode(['error' => 'Impossible de détecter les modèles Moonshot disponibles']);
    exit();
  }
} else {
  $model = $input['model'];
...
// Appel à l'API Moonshot (compatible OpenAI)
