...
if (empty($PERPLEXITY_API_KEY)) {
  ...
  echo json_encode(['error' => 'Clé API Perplexity non configurée']);
  exit();
}
...
if (!isset($input['model']) || empty($input['model'])) {
  ...
  $modelsData = getPerplexityModels($PERPLEXITY_API_KEY);
  ...
  if (isset($modelsData['models']) && !empty($modelsData['models'])) {
    ...
    http_response_code(500);
    echo json_encode(['error' => 'Impossible de détecter les modèles Perplexity disponibles']);
    exit();
  }
} else {
  $model = $input['model'];
...
// Appel à l'API Perplexity (compatible OpenAI)
