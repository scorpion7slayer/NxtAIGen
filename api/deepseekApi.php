...
if (empty($DEEPSEEK_API_KEY)) {
  ...
  echo json_encode(['error' => 'Clé API DeepSeek non configurée']);
  exit();
}
...
if (!isset($input['model']) || empty($input['model'])) {
  ...
  $modelsData = getDeepSeekModels($DEEPSEEK_API_KEY);
  ...
  if (isset($modelsData['models']) && !empty($modelsData['models'])) {
    ...
    http_response_code(500);
    echo json_encode(['error' => 'Impossible de détecter les modèles DeepSeek disponibles']);
    exit();
  }
} else {
  $model = $input['model'];
